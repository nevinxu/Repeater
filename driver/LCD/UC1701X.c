...
void displayString(unsigned char x_start, unsigned char y_start,unsigned char *iString)
{
	unsigned char i = 0;
	unsigned char iCnt=0;
...
void DisplayRate(u8 x_start, u8 y_start, u16 rate)
{
  ...
  A=rate%10;
	...
	DisplayNum(x_start+60,y_start,A);
  if(rate >=10)
  {
		DisplayNum(x_start+40,y_start,T);
  }
  else
  {
		DisplayNum(x_start+40,y_start,0);
  }
  if(rate >=100)
  {
		DisplayNum(x_start+20,y_start,H);
  }
  else
  {
		DisplayNum(x_start+20,y_start,0);
  }
  if(rate >=1000)
  {
		DisplayNum(x_start,y_start,S);
  }
  else
  {
		DisplayNum(x_start,y_start,0);
  }
}
